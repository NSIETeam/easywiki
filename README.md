...
**Cloud (Production):**
```bash
# 1. Install deps + set up PostgreSQL
pip install -r requirements.txt
export DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/easywiki

# 2. Start (gunicorn recommended for production)
...
pip install fastapi uvicorn numpy pyjwt python-multipart bcrypt jieba sentence-transformers
...
**Default login:** `admin@local` — password shown in console on first run.
...
**云端生产环境：**
```bash
# 1. 安装依赖 + 配置 PostgreSQL
pip install -r requirements.txt
export DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/easywiki

# 2. 启动（生产环境建议用 gunicorn）
python -m uvicorn orgmind.main:app --host 0.0.0.0 --port 8080
```

**本地开发（SQLite，无需 PostgreSQL）：**
```bash
# 1. 安装 Python 依赖
pip install fastapi uvicorn numpy pyjwt python-multipart bcrypt jieba sentence-transformers
...
**默认登录账号：** `admin@local` — 首次启动时密码打印在控制台。
