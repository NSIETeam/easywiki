@echo off
chcp 65001 >nul
title EasyWiki — Installer
setlocal enabledelayedexpansion

set "APP_NAME=EasyWiki"
...
net session >nul 2>&1
if "!errorlevel!"=="0" (
    set "INSTALL_DIR=C:\Program Files\EasyWiki"
    set "IS_ADMIN=1"
) else (
    set "INSTALL_DIR=%LOCALAPPDATA%\EasyWiki"
    set "IS_ADMIN=0"
)
...
echo.
echo   ==============================================
echo     EasyWiki v%APP_VERSION% — Installation
echo     Agent-Driven Knowledge Management
echo   ==============================================
echo.
...
venv\Scripts\pip install fastapi "uvicorn[standard]" numpy pyjwt python-multipart openai tenacity bcrypt jieba sentence-transformers 2>nul
...
echo cd /d "%%~dp0"
echo echo Starting EasyWiki on http://localhost:%PORT%
echo start "" "%%~dp0venv\Scripts\python" -m uvicorn orgmind.main_sqlite:app --host 0.0.0.0 --port %PORT%
echo start http://localhost:%PORT%
echo pause
) > "!INSTALL_DIR!\EasyWiki.bat"

:: Desktop shortcut
powershell -c "$w=New-Object -ComObject WScript.Shell;$s=$w.CreateShortcut('!USERPROFILE!\Desktop\EasyWiki.lnk');$s.TargetPath='!INSTALL_DIR!\EasyWiki.bat';$s.Save()" 2>nul

:: Firewall
if "!IS_ADMIN!"=="1" netsh advfirewall firewall add rule name="EasyWiki" dir=in action=allow protocol=TCP localport=%PORT% >nul 2>&1

echo.
echo   ==============================================
echo     Installation complete!
echo.
echo     Start: Desktop "EasyWiki" shortcut
...
if errorlevel 2 exit /b 0
start "" "!INSTALL_DIR!\EasyWiki.bat"
